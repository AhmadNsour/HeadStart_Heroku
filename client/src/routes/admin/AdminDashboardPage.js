...
export default class AdminDashboardPage extends Component {
  ...
  getPendingAdmins() {
    axios.get("/get-pending-admins").then(response => {
  ...
  deletePost = _id => {
    ...
    axios
      .post("/delete-user-post", { _id, user_id })
  ...
  getPendingTechQuestions() {
    axios
      .get("/get-tech-pendings")
      .then(res => this.setState({ pendingQuestions: res.data }));
  }

  answerPending = post => {
    axios.post("/answer-pending", post).then(res => {
  ...
  acceptPending = admin => {
    axios
      .post("/accept-pending", admin)
      .then(res => this.getPendingAdmins());
  };

  deletePending = _id => {
    axios.post("/delete-pending", { _id }).then(res => {
      if (this.props.loggedInUser.role === "owner") {
        this.getPendingAdmins();
      } else if (this.props.loggedInUser.role === "hrAdmin") {
        this.getPendingHrQuestions();
